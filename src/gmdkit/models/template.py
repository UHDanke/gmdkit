...
class SmartPrefab(PlistLoaderMixin,DictClass[int,Any],metaclass=FieldMetaclass):
    ...
    object_string: ObjectGroup = DictField(
        key=1,
        decoder=ObjectGroup,
        encoder=get_string,
        default_factory=ObjectGroup,
        is_node=True,
        pass_kwargs=True,
    )

    weight: int = DictField(
        key=2,
        default_factory=int,
    )
...
class SmartTemplate(FilePathMixin,PlistLoaderMixin,DictClass[int,Any],metaclass=FieldMetaclass):
    ...
    variations: SmartPrefabLayout = DictField(
        key=3,
        decoder=SmartPrefabLayout.from_node,
        encoder=SmartPrefabLayout.to_node,
        default_factory=SmartPrefabLayout,
        is_node=True,
        pass_kwargs=True
    )

    allow_rotation: bool = DictField(
        key=4,
        default_factory=bool,
    )

    allow_flip_x: bool = DictField(
        key=5,
        default_factory=bool,
    )

    allow_flip_y: bool = DictField(
        key=6,
        default_factory=bool,
    )

    ignore_corners: bool = DictField(
        key=7,
        default_factory=bool,
    )
